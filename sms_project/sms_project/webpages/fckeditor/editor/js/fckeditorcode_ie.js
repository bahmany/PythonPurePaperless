...
FCKDataProcessor.prototype = {
    ConvertToHtml: function(A) {
        if (FCKConfig.FullPage) {
            ...
            if (!FCKRegexLib.HtmlOpener.test(A)) A = '<html dir="rtl">' + A + '</html>';
            if (!FCKRegexLib.HeadOpener.test(A)) A = A.replace(FCKRegexLib.HtmlOpener, '$&<head><title></title></head>');
            return A;
        } else {
            var B = FCKConfig.DocType + '<html dir="rtl"';
...
var FCK = {
    ...
    Preview: function() {
        ...
        if (FCKConfig.FullPage) {
            ...
            A = FCKConfig.DocType + '<html dir="rtl"><head>' + FCK.TempBaseTag + '<title>' + FCKLang.Preview + '</title>' + _FCK_GetEditorAreaStyleTags() + '</head><body' + FCKConfig.GetBodyAttributes() + '>' + FCK.GetXHTML() + '</body></html>';
        };
...
FCKXHtml._RemoveXHtmlJobProperties = function(A) {
    ...
    if (typeof A._fckxhtmljob !== 'undefined') A.removeAttribute('_fckxhtmljob');
    if (A.hasChildNodes()) {
        var B = A.childNodes;
        for (var i = B.length - 1; i >= 0; i--) FCKXHtml._RemoveXHtmlJobProperties(B.item(i));
    }
};
...
var FCKJustifyCommand = function(A) {
    ...
    this.IsDefaultAlign = (A == 'left' && B == 'rtl') || (A == 'right' && B == 'rtl');
...
var FCKIndentCommand = function(A, B) {
    this.Name = A;
    this.Offset = B;
    this.IndentCSSProperty = FCKConfig.ContentLangDirection.IEquals('rtl') ? 'marginLeft' : 'marginRight';
};
...
FCKToolbar.prototype.Create = function(A) {
    ...
    e.style.styleFloat = e.style.cssFloat = (FCKLang.Dir == 'rtl' ? 'left' : 'right');
    e.dir = FCKLang.Dir;
    e.cellPadding = 0;
    e.cellSpacing = 0;
    var C = e.insertRow(-1);
