...
def encode(str):
    s=""
    s=triple_des('This@@si*&_&!Sec').encrypt(str, padmode=2)
    return s

def decode(str):
    s=""
    s=triple_des('This@@si*&_&!Sec').decrypt(str, padmode=2)
    return s
...
def get_company_name_from_user_id(user_id):
    return executeAndReturnRows("select socc_name from tbl_sms_owner_companies_clients where socc_id=(select u_soccc_id from tbl_users where u_id={0})".format(user_id))[0][0]





def unhash_decode_ord_conv_to_arr(str):
    OOO = str.split("::")
    hashed=""
    for _ord in OOO:
